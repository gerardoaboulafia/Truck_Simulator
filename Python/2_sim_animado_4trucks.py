...
# • Todos los camiones parten con un trailer cargado.
# • Cada ciclo: descargar en CD → elegir planta → vacío → carga → CD.
# • Inventario fijo: 9 semiacoplados en total.
# ---------------------------------------------------------------------
...
def run_sim(hours: float = 8,
        ...
        def process(self):
            while True:
                ...
                if self.carry is not None:
                    ...
                    dock_busy_time[0] += env.now() - start
                    self.trips += 1
                    wake()
                ...
                pid = self.choose()
                if pid is None:
                    t0 = env.now(); yield self.passivate()
                    dt = env.now() - t0; self.wait_cd += dt
                ...
                dt = env.now() - t0; self.wait_pl += dt
                if dt>0: self.no_full += 1
                yield self.hold(hook_t.sample())
                yield from self._move(CD_PT[pid], DIST[pid]/speed_dist.sample())
                # al volver al bucle descargará de nuevo
    ...
    trucks.append(Truck(name="T4", idx=3, home_pid=0, first_trailer=t4_tr, x_shift=20))
...
if __name__ == "__main__":
    res = run_sim(hours=8, beta=1, seed=42, debug=False, animate=True)
    print("\nResumen:", res)